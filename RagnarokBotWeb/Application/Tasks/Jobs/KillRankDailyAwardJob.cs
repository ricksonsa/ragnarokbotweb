...
{
    public class KillRankDailyAwardJob(
        ...
        ) : AbstractJob(scumServerRepository), IJob
    {
        public async Task Execute(IJobExecutionContext context)
        {
            ...
            try
            {
                ...
                await HandleAwardsDaily(unitOfWork, discordService, server, topKillersDaily, manager);
            }
        ...
        {
            if (server.KillRankDailyTop1Award.HasValue && server.KillRankDailyTop1Award.Value > 0)
            {
                ...
                var amount = server.KillRankDailyTop1Award.Value;
                ...
                if (player != null)
                {
                    ...
                    if (player.DiscordId.HasValue)
                    {
                        ...
                        embed.Text = $"You secured the Top 1 spot in the Daily Kill Ranking.\r\nAs a reward you’ve earned 💰 {amount} Coins! 🔥\r\n\r\n";
            ...
            if (server.KillRankDailyTop2Award.HasValue && server.KillRankDailyTop2Award.Value > 0)
            {
                var player = await uow.Players
                    .Include(p => p.ScumServer)
                    .FirstOrDefaultAsync(p => p.SteamId64 == topKillers[0].SteamId && p.ScumServerId == server.Id);
                var amount = server.KillRankDailyTop2Award.Value;
                ...
                if (player != null)
                {
                    ...
                    if (player.DiscordId.HasValue)
                    {
                        var embed = new CreateEmbed(player.DiscordId.Value);
                        embed.Title = "🏆 Congratulations! 🏆";
                        embed.Text = $"You secured the Top 2 spot in the Daily Kill Ranking.\r\nAs a reward you’ve earned 💰 {amount} Coins! 🔥\r\n\r\n";
                        embed.Color = Color.DarkOrange;
                        await discordService.SendEmbedToUserDM(embed);
                    }
                }
            ...
            if (server.KillRankDailyTop3Award.HasValue && server.KillRankDailyTop3Award.Value > 0)
            {
                var player = await uow.Players
                    .Include(p => p.ScumServer)
                    .FirstOrDefaultAsync(p => p.SteamId64 == topKillers[0].SteamId && p.ScumServerId == server.Id);
                var amount = server.KillRankDailyTop3Award.Value;
                ...
                if (player != null)
                {
                    ...
                    if (player.DiscordId.HasValue)
                    {
                        var embed = new CreateEmbed(player.DiscordId.Value);
                        embed.Title = "🏆 Congratulations! 🏆";
                        embed.Text = $"You secured the Top 3 spot in the Daily Kill Ranking.\r\nAs a reward you’ve earned 💰 {amount} Coins! 🔥\r\n\r\n";
                        embed.Color = Color.DarkOrange;
                        await discordService.SendEmbedToUserDM(embed);
                    }
                }
            ...
            if (server.KillRankDailyTop4Award.HasValue && server.KillRankDailyTop4Award.Value > 0)
            {
                var player = await uow.Players
                    .Include(p => p.ScumServer)
                    .FirstOrDefaultAsync(p => p.SteamId64 == topKillers[0].SteamId && p.ScumServerId == server.Id);
                var amount = server.KillRankDailyTop4Award.Value;
                ...
                if (player != null)
                {
                    ...
                    if (player.DiscordId.HasValue)
                    {
                        var embed = new CreateEmbed(player.DiscordId.Value);
                        embed.Title = "🏆 Congratulations! 🏆";
                        embed.Text = $"You secured the Top 4 spot in the Daily Kill Ranking.\r\nAs a reward you’ve earned 💰 {amount} Coins! 🔥\r\n\r\n";
                        embed.Color = Color.DarkOrange;
                        await discordService.SendEmbedToUserDM(embed);
                    }
                }
            ...
            if (server.KillRankDailyTop5Award.HasValue && server.KillRankDailyTop5Award.Value > 0)
            {
                var player = await uow.Players
                    .Include(p => p.ScumServer)
                    .FirstOrDefaultAsync(p => p.SteamId64 == topKillers[0].SteamId && p.ScumServerId == server.Id);
                var amount = server.KillRankDailyTop5Award.Value;
                ...
                if (player != null)
                {
                    ...
                    if (player.DiscordId.HasValue)
                    {
                        var embed = new CreateEmbed(player.DiscordId.Value);
                        embed.Title = "🏆 Congratulations! 🏆";
                        embed.Text = $"You secured the Top 5 spot in the Daily Kill Ranking.\r\nAs a reward you’ve earned 💰 {amount} Coins! 🔥\r\n\r\n";
                        embed.Color = Color.DarkOrange;
                        await discordService.SendEmbedToUserDM(embed);
                    }
                }
